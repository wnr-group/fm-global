...
**Goal:** Rebuild the training page with Industrial Editorial aesthetic showcasing Mechanical Technician and Permit Receiver courses with cinematic imagery, bold typography, and layered content reveal.

**Architecture:** Single-page redesign replacing existing training page content. Course data defined as typed constants. Sections alternate dark/light backgrounds for visual rhythm. Reuses existing ScrollReveal, AnimatedCounter, Button, and MobileNav components.

**Tech Stack:** Next.js 16, React 19, Tailwind CSS 4, TypeScript, existing GSAP-based scroll animations

**Design Principles (Impeccable):**
- Industrial Editorial tone: magazine-style layout meets technical spec sheet
- NO AI slop: no glassmorphism, no gradient text, no identical card grids, no centered-everything layouts
- Asymmetric compositions, varied spacing rhythm, left-aligned content
- Tinted neutrals (azure-tinted darks, not pure black)
- Motion: staggered reveals with exponential easing, transform/opacity only
...
| File | Responsibility |
|------|----------------|
| `public/training/` | 6 course images (new directory) |
| `src/app/training/page.tsx` | Complete page rewrite with all sections |
| `src/app/globals.css` | New grain overlay animation, industrial accent utilities |
...
**Files:**
- Create: `public/training/hero-refinery-dusk.png`
- Create: `public/training/course-mechanical-technician.png`
- Create: `public/training/course-permit-receiver.png`
- Create: `public/training/career-aerial-facility.png`
- Create: `public/training/training-transformation.png`
- Create: `public/training/gulf-skyline.png`

- [ ] **Step 1: Create training images directory**

Run:
```bash
mkdir -p public/training
```

- [ ] **Step 2: Copy and rename images from Downloads**

Run:
```bash
cp ~/Downloads/Gemini_Generated_Image_37euxa37euxa37eu.png public/training/hero-refinery-dusk.png
cp ~/Downloads/Gemini_Generated_Image_vs6xedvs6xedvs6x.png public/training/course-mechanical-technician.png
cp ~/Downloads/Gemini_Generated_Image_m52or4m52or4m52o.png public/training/course-permit-receiver.png
cp ~/Downloads/Gemini_Generated_Image_z8gw8zz8gw8zz8gw.png public/training/career-aerial-facility.png
cp ~/Downloads/Gemini_Generated_Image_3oaun3oaun3oaun3.png public/training/training-transformation.png
cp ~/Downloads/Gemini_Generated_Image_2cs8xo2cs8xo2cs8.png public/training/gulf-skyline.png
```
...
Run:
```bash
git add public/training/
git commit -m "feat: add training page images for industrial editorial redesign"
```

---

## Task 2: Add CSS Utilities for Industrial Aesthetic

**Files:**
- Modify: `src/app/globals.css` (append at end)

- [ ] **Step 1: Add grain overlay and industrial utilities to globals.css**

Append to end of `src/app/globals.css`:

```css
/* ==========================================================================
   INDUSTRIAL EDITORIAL - Training Page Utilities
   ========================================================================== */
...
/* Industrial accent line - left edge */
.accent-line-industrial {
  position: relative;
}

.accent-line-industrial::before {
  content: "";
  position: absolute;
  left: 0;
  top: 2rem;
  bottom: 2rem;
  width: 3px;
  background: linear-gradient(180deg, transparent 0%, oklch(0.6 0.15 45) 20%, oklch(0.6 0.15 45) 80%, transparent 100%);
}

.accent-line-green::before {
  background: linear-gradient(180deg, transparent 0%, oklch(0.52 0.14 130) 20%, oklch(0.52 0.14 130) 80%, transparent 100%);
}

/* Dark section backgrounds - azure tinted, not pure black */
.bg-industrial-dark {
  background-color: oklch(0.15 0.02 240);
}

.bg-industrial-darker {
  background-color: oklch(0.12 0.025 240);
}

/* Stats bar dark strip */
.bg-stats-strip {
  background: linear-gradient(90deg, oklch(0.14 0.02 240) 0%, oklch(0.18 0.025 240) 50%, oklch(0.14 0.02 240) 100%);
}

/* Syllabus grid numbering */
.syllabus-number {
...
/* Tag/chip styles - industrial, not rounded colorful */
.tag-industrial {
  display: inline-flex;
  align-items: center;
...
.tag-industrial-light {
  background: oklch(0.94 0.008 240);
  color: oklch(0.35 0.04 240);
  border: 1px solid oklch(0.88 0.01 240);
}

/* Interview support highlight box */
.highlight-box {
  background: linear-gradient(135deg, oklch(0.52 0.14 130 / 0.1) 0%, oklch(0.52 0.14 130 / 0.05) 100%);
  border-left: 3px solid oklch(0.52 0.14 130);
}
...
/* Exponential easing for scroll reveals */
.ease-out-expo {
...
Run:
```bash
cd "/Users/dineshlearning/Documents/make money/fm-global/website" && npm run build 2>&1 | head -20
```
...
git commit -m "feat: add industrial editorial CSS utilities for training page"
```

---

## Task 3: Define Course Data Types and Constants

**Files:**
- Modify: `src/app/training/page.tsx` (replace existing course data)

- [ ] **Step 1: Replace entire training page with new structure and data**

Replace the entire content of `src/app/training/page.tsx` with:
...
- [ ] **Step 2: Run development server to check for errors**

Run:
```bash
cd "/Users/dineshlearning/Documents/make money/fm-global/website" && npm run dev 2>&1 &
sleep 5 && curl -s -o /dev/null -w "%{http_code}" http://localhost:3000/training
```
...
git add src/app/training/page.tsx src/app/globals.css
git commit -m "feat: complete training page redesign with industrial editorial aesthetic

- Full-viewport cinematic hero with refinery dusk image
- Course overview cards with hover effects
- Detailed course sections (Mechanical Technician, Permit Receiver)
- Why Choose FM Institute section with transformation imagery
- Career Destinations with Gulf countries
- Final CTA with aerial facility background
- Industrial utilities: grain overlay, accent lines, tags"
```

---

## Task 4: Visual QA and Refinements

**Files:**
- Modify: `src/app/training/page.tsx` (if needed)
...
Run:
```bash
cd "/Users/dineshlearning/Documents/make money/fm-global/website" && npm run dev
```
...
- [ ] Hero: Full viewport, stats visible, scroll indicator present
- [ ] Course Overview: Two cards, hover effects working
- [ ] Mechanical Technician: Dark background, image on right, orange accent line
- [ ] Permit Receiver: Light background, image on left, green accent line
- [ ] Why Choose: Background image visible, features grid readable
- [ ] Career Destinations: Skyline image, stats bar dark
- [ ] CTA: Aerial image, centered content, trust signals

- [ ] **Step 3: Mobile responsiveness check**

Resize browser to mobile width (~375px):
- [ ] Hero content readable, not overflowing
- [ ] Course cards stack vertically
- [ ] Syllabus grid single column
...
```bash
git add -A
git commit -m "fix: visual refinements for training page" --allow-empty
```
...
**1. Spec coverage:**
- [x] Section 1: Hero - Task 3 (HeroSection component)
- [x] Section 2: Course Overview - Task 3 (CourseOverviewSection)
- [x] Section 3: Mechanical Technician - Task 3 (CourseDetailSection variant="dark")
- [x] Section 4: Permit Receiver - Task 3 (CourseDetailSection variant="light")
- [x] Section 5: Why Choose - Task 3 (WhyChooseSection)
- [x] Section 6: Career Destinations - Task 3 (CareerDestinationsSection)
- [x] Section 7: CTA - Task 3 (CTASection)
- [x] Section 8: Footer - Task 3 (Footer component)
- [x] Images - Task 1

**2. Placeholder scan:** No TBDs, TODOs, or "implement later" found.

**3. Type consistency:**
- Course interface used consistently
- Component props match definitions
- All icon imports from lucide-react verified
...
|------|-------------|-----------|
| 1 | Copy images to public directory | 2 min |
| 2 | Add CSS utilities for industrial aesthetic | 3 min |
| 3 | Complete page rewrite with all sections | 5 min |
| 4 | Visual QA and refinements | 5 min |
